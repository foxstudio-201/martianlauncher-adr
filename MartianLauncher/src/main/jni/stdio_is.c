...
Java_com_foxstudio_martianlauncher_bridge_LoggerBridge_start(JNIEnv *env, __attribute((unused)) jclass clazz, jstring logPath) {
    if (latestlog_fd != -1)
    {
    ...
    {
        jclass eventLogListener = (*env)->FindClass(env, "com/foxstudio/martianlauncher/bridge/LoggerBridge$EventLogListener");
...
JNIEXPORT void JNICALL Java_com_foxstudio_martianlauncher_bridge_LoggerBridge_append(JNIEnv *env, __attribute((unused)) jclass clazz, jstring text) {
...
JNIEXPORT void JNICALL
Java_com_foxstudio_martianlauncher_bridge_LoggerBridge_setListener(JNIEnv *env, __attribute((unused)) jclass clazz, jobject log_listener) {
...
JNIEXPORT void JNICALL
Java_com_foxstudio_martianlauncher_bridge_ZLBridge_setupExitMethod(JNIEnv *env, jclass clazz,
                                                        jobject context) {
    exitTrap_ctx = (*env)->NewGlobalRef(env,context);
    (*env)->GetJavaVM(env,&exitTrap_jvm);
    exitTrap_exitClass = (*env)->NewGlobalRef(env,(*env)->FindClass(env,"com/foxstudio/martianlauncher/bridge/ZLNativeInvoker"));
