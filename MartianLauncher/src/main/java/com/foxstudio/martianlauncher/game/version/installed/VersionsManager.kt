...
package com.foxstudio.martianlauncher.game.version.installed

import com.foxstudio.martianlauncher.BuildKeys
import com.foxstudio.martianlauncher.game.launch.LogName
import com.foxstudio.martianlauncher.game.path.getVersionsHome
import com.foxstudio.martianlauncher.game.version.installed.utils.parseJsonToVersionInfo
...
object VersionsManager {
    ...
    /**
     * @return 获取 Martian 启动器版本标识文件夹
     */
    fun getMartianVersionPath(version: Version) = File(version.getVersionPath(), BuildKeys.LAUNCHER_IDENTIFIER)

    /**
     * @return 通过目录获取 Martian 启动器版本标识文件夹
     */
    fun getMartianVersionPath(folder: File) = File(folder, BuildKeys.LAUNCHER_IDENTIFIER)

    /**
     * @return 通过名称获取 Martian 启动器版本标识文件夹
     */
    fun getMartianVersionPath(name: String) = File(getVersionPath(name), BuildKeys.LAUNCHER_IDENTIFIER)

    /**
     * @return 游戏的上一次运行日志
     */
    fun getLatestLog(version: Version) = File(getMartianVersionPath(version), LogName.GAME.fileName)

    /**
     * @return 获取当前版本设置的图标
     */
    fun getVersionIconFile(version: Version) = File(getMartianVersionPath(version), "VersionIcon.png")

    /**
     * @return 通过目录获取 Martian 启动器版本标识文件夹
     */
    fun getVersionIconFile(folder: File) = File(getMartianVersionPath(folder), "VersionIcon.png")

    /**
     * @return 通过名称获取当前版本设置的图标
     */
    fun getVersionIconFile(name: String) = File(getMartianVersionPath(name), "VersionIcon.png")
