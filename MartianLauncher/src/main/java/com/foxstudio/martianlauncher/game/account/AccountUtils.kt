...
package com.foxstudio.martianlauncher.game.account
...
import com.foxstudio.martianlauncher.context.COPY_LABEL_DEVICE_CODE
...
import com.foxstudio.martianlauncher.game.account.auth_server.getAuthServeInfo
import com.foxstudio.martianlauncher.game.account.microsoft.AsyncStatus
import com.foxstudio.martianlauncher.game.account.microsoft.AuthType
...
import com.foxstudio.martianlauncher.game.account.microsoft.microsoftAuthAsync
...
fun microsoftLogin(
    ...
    val task = Task.runTask(
        ...
        task = { task ->
            //打开微软登录网页，等待登录完成后拦截到的授权码（参考 VoxelXClient 的网页登录方式）
            task.updateProgress(-1f, R.string.account_microsoft_get_token)
            val deferred = MicrosoftLoginSession.start()
            withContext(Dispatchers.Main) {
                toWeb(MicrosoftOAuth.authorizeUrl())
            }
            val code = deferred.await() ?: run {
                //用户关闭了登录网页，视为取消登录
                withContext(Dispatchers.Main) {
                    Toast.makeText(
                        context,
                ...
                throw CancellationException("Microsoft login cancelled by user")
            }
            withContext(Dispatchers.Main) {
                backToMain()
            }
            //用授权码换取令牌
            val tokenResponse = MicrosoftOAuth.exchangeAuthCode(code, coroutineContext)
