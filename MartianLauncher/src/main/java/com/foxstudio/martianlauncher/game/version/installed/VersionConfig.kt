...
import com.foxstudio.martianlauncher.game.version.installed.VersionsManager.getMartianVersionPath
...
import com.foxstudio.martianlauncher.utils.string.getStringNotNull
import java.io.File
...
class VersionConfig(
    ...
    fun saveWithThrowable() {
        val martianVersionPath = getMartianVersionPath(versionPath)
        val configFile = File(martianVersionPath, "version.config")
        if (!martianVersionPath.exists()) martianVersionPath.mkdirs()
    ...
    companion object CREATOR : Parcelable.Creator<VersionConfig> {
        ...
        fun parseConfig(versionPath: File): VersionConfig {
            val configFile = File(getMartianVersionPath(versionPath), "version.config")
